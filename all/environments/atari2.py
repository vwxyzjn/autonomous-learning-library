...
class Atari2Environment(GymEnvironment):
    def __init__(self, name, *args, **kwargs):
        # need these for duplication
        self._args = args
        self._kwargs = kwargs
        # construct the environment
        original_env = gym.make(name)
    ...
    def duplicate(self, n):
        return [
            Atari2Environment(self._name, *self._args, **self._kwargs) for _ in range(n)
        ]
    ...
    def _make_state(self, raw, done, info=None):
        ...
        return State(
            torch.from_numpy(
                np.array(raw, dtype=self.state_space.dtype)
            )
            .unsqueeze(0)
            .to(self._device),
